...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<UserDto> {    
    if (createUserDto.roleIds) {
      ...
      roles.forEach(role => {
        const roleUserObject = { roleId: role.id, userId: user.id, isActive: true }
        listOfInserts.push(roleUserObject);
      });
      const roleUsers = await this.roleUserRepository.createQueryBuilder("RoleUser")
        .insert()
        .into("role_users")
  ...
  async addRoles(addRolesDto: UserRolesDto): Promise<UserDto>{
    ...
    const user = await this.userRepository.findOne({ where: { id: addRolesDto.userId }});
    if(!rolesToAdd || !user) throw new HttpException(ERRORS.User_Errors.ERR005, HttpStatus.NOT_FOUND);
    ...
    const insertRoles = await this.roleUserRepository.createQueryBuilder("role_users")
      .insert()
      .into("role_users")
      .values(listOfInserts)
      .execute();
  ...
  async deleteRoles(userRolesDto: UserRolesDto): Promise<UserDto>{
    ...
    const user = await this.userRepository.findOne({ where: { id: userRolesDto.userId }});
    if(!roles || !user) throw new HttpException(ERRORS.User_Errors.ERR005, HttpStatus.NOT_FOUND);

    const roleUser = await this.roleUserRepository.createQueryBuilder("roleUser")
      .update(RoleUser)
      .set({ isActive: false })
  ...
  async getRoleList(userId: number): Promise<RoleDto[]>{
    ...
    const roleUsers = await this.roleUserRepository.find({ where: {
        userId: userId,
        isActive: true
      },
      relations: { role: true }
